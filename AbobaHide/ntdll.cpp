...
NTSTATUS NTDLL::Initialize()
{
    ...
    {
#ifdef _DEBUG
        DbgPrint("[ABOBAHIDE] KeGetCurrentIrql != PASSIVE_LEVEL!\n");
#endif
    ...
    if(NT_SUCCESS(NtStatus))
    {
        ...
        if(NT_SUCCESS(NtStatus))
        {
            ...
            Log("[ABOBAHIDE] FileSize of ntdll.dll is %08X!\r\n", StandardInformation.EndOfFile.LowPart);
            ...
            if(!NT_SUCCESS(NtStatus))
            {
                ...
                Log("[ABOBAHIDE] ZwReadFile failed with status %08X...\r\n", NtStatus);
            }
        }
        else
            Log("[ABOBAHIDE] ZwQueryInformationFile failed with status %08X...\r\n", NtStatus);
        ZwClose(FileHandle);
    }
    else
        Log("[ABOBAHIDE] ZwCreateFile failed with status %08X...\r\n", NtStatus);
    return NtStatus;
}
...
int NTDLL::GetExportSsdtIndex(const char* ExportName)
{
    ...
    if(SsdtOffset == -1)
    {
        Log("[ABOBAHIDE] SSDT Offset for %s not found...\r\n", ExportName);
    }
