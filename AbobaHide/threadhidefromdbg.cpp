...
NTSTATUS FindCrossThreadFlagsOffset(_Out_ PULONG Offset)
{
    *Offset = 0;
    ...
    if(BuildNumber < 6000)
    {
        ...
        {
            Log("[ABOBAHIDE] FindCrossThreadFlagsOffset: unsupported OS!\r\n");
    ...
    if(MatchesFound != 1)
    {
        Log("[ABOBAHIDE] Failed to find reliable match for CrossThreadFlags offset: wanted 1 match, found %u.\r\n", MatchesFound);
        Status = STATUS_NOT_FOUND;
        goto Exit;
    }

    *Offset = LastMatchFound;
    Log("[ABOBAHIDE] Found CrossThreadFlags at offset +0x%04X. 'HideFromDebugger' will be stripped from running threads in target processes.\r\n", *Offset);
...
NTSTATUS UndoHideFromDebuggerInRunningThreads(_In_ ULONG Pid)
{
            ...
            for(ULONG i = 0; i < Entry->NumberOfThreads; ++i)
            {
                ...
                if(NT_SUCCESS(Status))
                {
                    ...
                    if((InterlockedAnd(CrossThreadFlagsAddress, ~PS_CROSS_THREAD_FLAGS_HIDEFROMDBG) & PS_CROSS_THREAD_FLAGS_HIDEFROMDBG) != 0)
                    {
                        ...
                        Log("[ABOBAHIDE] Stripped ThreadHideFromDebugger flag from PID %u, TID %u!\r\n",
    ...
    if(!Found)
    {
        Log("[ABOBAHIDE] PID %u was not found in the process list!\r\n", Pid);
    }
    else
    {
        if(NumThreadFlagsStripped == 0)
            Log("[ABOBAHIDE] PID %u does not have any threads with the ThreadHideFromDebugger flag set.\r\n", Pid);
        else
            Log("[ABOBAHIDE] Stripped ThreadHideFromDebugger flag from %u threads in PID %u!\r\n", NumThreadFlagsStripped, Pid);
    }
