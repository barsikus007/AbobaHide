...
static void AbobaHideCall(DWORD ProcessId, HIDE_COMMAND Command)
{
...
extern "C" __declspec(dllexport) void _ODBG_Pluginmainloop(DEBUG_EVENT* DebugEvent)
{
    ...
    case CREATE_PROCESS_DEBUG_EVENT:
    {
        ...
        AbobaHideCall(ProcessId, HidePid);
        PEBHidden = false;
    }
    break;
    ...
    case EXIT_PROCESS_DEBUG_EVENT:
    {
        if(DebugEvent->dwProcessId == ProcessId) //main process terminates
            AbobaHideCall(ProcessId, UnhidePid);
    }
    break;
    }
}
...
{
    wcscpy_s(pluginname, 32, L"AbobaHide");
