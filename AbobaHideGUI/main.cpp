...
static void AbobaHideCall(HWND hwndDlg, HIDE_COMMAND Command)
{
    HANDLE hDevice = CreateFileA("\\\\.\\AbobaHide", GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING, 0, 0);
    if(hDevice == INVALID_HANDLE_VALUE)
    {
        MessageBoxA(hwndDlg, "Could not open AbobaHide handle...", "Driver loaded?", MB_ICONERROR);
        return;
    }
    HIDE_INFO HideInfo;
...
static BOOL CALLBACK DlgMain(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        case IDC_BTN_HIDE:
        {
            AbobaHideCall(hwndDlg, HidePid);
        }
        return TRUE;

        case IDC_BTN_UNHIDE:
        {
            AbobaHideCall(hwndDlg, UnhidePid);
        }
        return TRUE;

        case IDC_BTN_UNHIDEALL:
        {
            AbobaHideCall(hwndDlg, UnhideAll);
        }
        return TRUE;
        }
    }
